...
def compareWithTable(request):
    ...
    fitsWithTable = FP.compareColumns(fileDF, request.data['schema'], request.data['tableName'])
    if fitsWithTable:
...
def getSubjectList(request):
    ...
    returnVal, msgStr, subjectDF = AdminPkg.getSubjectList(subjectDF=subjectDF)
